...
class Evaluator:
    def __init__(
        ...
        no_overlapping: bool, predictions_path: str, epoch: int,
        dataset_label: str): # noqa
    ...
    def eval_batch(self, batch_entity_clf: torch.tensor, batch_rel_clf: torch.tensor, batch_rels: torch.tensor, batch: dict):
        ...
        for i in range(batch_size):
            ...
            rel_nonzero = torch.nonzero(rel_clf, as_tuple=False).view(-1)
            ...
            sample_pred_relations = self._convert_pred_relations(rel_types, rel_entity_spans, rel_entity_types, rel_scores)

            # get entities that are not classified as 'None'
            # valid_entity_indices = entity_types.nonzero().view(-1)
            valid_entity_indices = torch.nonzero(entity_types, as_tuple=False).view(-1)
            valid_entity_types = entity_types[valid_entity_indices]
            valid_entity_spans = batch['entity_spans'][i][valid_entity_indices]
            valid_entity_scores = torch.gather(batch_entity_clf[i][valid_entity_indices], 1, valid_entity_types.unsqueeze(1)).view(-1)

            sample_pred_entities = self._convert_pred_entities(valid_entity_types, valid_entity_spans, valid_entity_scores)

            if self._no_overlapping:
                sample_pred_entities, sample_pred_relations = self._remove_overlapping(sample_pred_entities, sample_pred_relations)
    ...
    def store_predictions(self):
        ...
        for i, doc in enumerate(self._dataset.documents):
            ...
            doc_predictions = dict(tokens=[t.phrase for t in tokens], entities=converted_entities, relations=converted_relations)
    ...
    def _convert_gt(self, docs: List[Document]):
        for doc in docs:
            ...
            if self._no_overlapping:
                sample_gt_entities, sample_gt_relations = self._remove_overlapping(sample_gt_entities, sample_gt_relations)
