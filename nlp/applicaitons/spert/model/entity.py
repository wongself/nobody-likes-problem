...
class Relation:
    def __init__(self, rid: int, relation_type: RelationType, head_entity: Entity, tail_entity: Entity, reverse: bool = False):
    ...
    def as_tuple(self):
        ...
        t = ((head_start, head_end, head.entity_type), (tail_start, tail_end, tail.entity_type), self._relation_type)
        return t
...
class Document:
    def __init__(self, doc_id: int, tokens: List[Token], entities: List[Entity], relations: List[Relation], encoding: List[int]):
...
class BatchIterator:
    ...
    def __next__(self):
        if self._truncate and self._i + self._batch_size > self._length:
            ...
            entities = [
                self._entities[n]
                for n in self._order[self._i:self._i + self._batch_size]
            ]
...
class Dataset(TorchDataset):
    ...
    def __init__(self, label, rel_types, entity_types, neg_entity_count, neg_rel_count, max_span_size):
        self._label = label
    ...
    def create_document(self, tokens, entity_mentions, relations,
                        doc_encoding) -> Document:
        document = Document(self._doc_id, tokens, entity_mentions, relations,
                            doc_encoding)
    ...
    def create_relation(self,
                        ...
                        head_entity,
                        tail_entity,
                        reverse=False) -> Relation:
        relation = Relation(self._rid, relation_type, head_entity, tail_entity,
                            reverse)
    ...
    def __getitem__(self, index: int):
        ...
        if self._mode == Dataset.TRAIN_MODE:
            return sampling.create_train_sample(doc, self._neg_entity_count, self._neg_rel_count, self._max_span_size, len(self._rel_types))
        else:
