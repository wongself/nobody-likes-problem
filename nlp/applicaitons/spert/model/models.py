...
class SpERT(BertPreTrainedModel):
    ...
    def __init__(
        self, config: BertConfig, cls_token: int,
        entity_types: int, relation_types: int,
    ...
    def _classify_entities(self, encodings, h, entity_masks, size_embeddings):
        ...
        entity_repr = torch.cat(
            [entity_ctx.unsqueeze(1).repeat(1, entity_spans_pool.shape[1], 1), entity_spans_pool, size_embeddings], dim=2)
    ...
    def _filter_spans(self, entity_clf, entity_spans, entity_sample_masks, ctx_size):
        ...
        for i in range(batch_size):
            ...
            non_zero_indices = torch.nonzero((entity_logits_max[i] != 0), as_tuple=False).view(-1)
