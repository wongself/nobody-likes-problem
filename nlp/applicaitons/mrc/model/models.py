...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, mask, l, blks):  # blks 应该是block的个数 l还未知（感觉还是不要动为妙，1）
        total_layers = (self.conv_num+1)*blks
        out = PosEncoder(x)  # PosEncoder不知道是个啥
        for i, conv in enumerate(self.convs):
            res = out
            out = self.norm_C[i](out.transpose(1,2)).transpose(1,2)  #这里是第i层的layernorm来规范化
            if (i) % 2 == 0:
...
class QANet(nn.Module):
    ...
    def forward(self, Cwid, Ccid, Qwid, Qcid):
        ...
        C, Q = self.emb(Cc, Cw, Lc), self.emb(Qc, Qw, Lq)  # 这里要把char和word合并一起，char是64维，word是300维，默认1个word有16个char
