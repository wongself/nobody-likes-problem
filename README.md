...
>   情感分析模块
7. ~~位于项目根目录，输入命令`python ./nlp/applicaitons/sana/server.py`来启动翻译模块，随后在浏览器中输入本机网址及端口`2337`，来测试模块是否启动成功。若页面出现出现`Nobody Likes Problem`，则表明模块启动成功。~~ 翻译模块已经在端口`2347`启动成功。
...
>  情感分析模块
7. 位于项目根目录，输入命令`python ./nlp/applicaitons/sana/server.py`来启动翻译模块。
